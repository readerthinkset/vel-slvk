"""
VELOCITY SLOVAK - Unified Social Media Upload Script
"""
...
def generate_caption(phrases, category, platform="facebook"):
    base = [f"Learn Slovak with VELOCITY SLOVAK!", "", f"Category: {category}", "", f"Master Slovak one phrase at a time! Today's {category} lesson:", ""]
    for i, p in enumerate(phrases[:5], 0):
        base.append(f"{i+1}. {p['english']}")
        base.append(f"   {p.get('slovak', '')}")
...
def upload_to_all_platforms(video_path, caption, category, phrases=None):
    ...
    print("\n" + "="*80)
    print(f"VELOCITY SLOVAK - MULTI-PLATFORM UPLOAD")
    print("="*80)
        ...
        if func:
            try:
                if pname == "youtube":
                    ...
                    r = func(video_path=video_path, description=caption, title=f"Slovak: {category}")
                elif pname == "telegram":
...
def main():
    print("\n" + "="*80)
    print(f"VELOCITY SLOVAK - AUTOMATED UPLOAD")
    print("="*80)
