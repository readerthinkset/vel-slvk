...
def upload_to_vk(video_path, description="", title=""):
    ...
    try:
        ...
        message = description or "Learn with VELOCITY SLOVAK!"
        video = upload.video(video_file=str(video_path), name=title or 'VELOCITY SLOVAK', description=description[:220] if description else '', group_id=group_id_int, wallpost=0)
